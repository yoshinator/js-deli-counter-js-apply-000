...
console.log(takeANumber(array, ticketNumber))
console.log(array)
console.log(takeANumber(array, ticketNumber))
console.log(array)
console.log(takeANumber(array, ticketNumber))
...
function currentLine(line){
  ...
  if (line.length){
    for (i=0; i < line.length; i++){
      temp[i] = ` ${i+1}. ${line[i]}` //we are replacing the original element with a new element. We prepend a space and an ordered number followed by a period and space then add the original element so that the array elements are formatted in the way we want. 
    }
